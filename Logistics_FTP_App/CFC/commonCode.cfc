	...
	<cfscript>
		...
		function schedulerLogLatestInfoFor(procName) {
			...
			if (FileExists(Request.schedulerLogPath)) {
				...
				Request.primitiveCode.safely_cfmail('Ray_Horn@redacted.com', Request.const_do_not_reply_symbol, 'Missing the file named "#Request.schedulerLogPath#"', '"#Request.schedulerLogPath#" is missing or cannot be found due to a bad file name.');
			}
			return _details;
		}
		...
		function tibcoFTPDownloader2LatestInfo() {
			...
			//	Request.primitiveCode.safely_cfmail('Ray_Horn@redacted.com', Request.const_do_not_reply_symbol, 'Missing the file named "#Request.tibcoFTPDownloader2ActivityPath#"', '"#Request.tibcoFTPDownloader2ActivityPath#" is missing or cannot be found due to a bad file name.');
			}
			return _msg;
		}
